...
export const galleryFragmentShader = /* glsl */ `
  ...
  void main() {
    ...
    vec3 gradedColor = mix(splitColor, vec3(1.0), 0.055);
    float topFromScreen = uViewportHeight - (gl_FragCoord.y / uPixelRatio);
    float topVisibility = smoothstep(155.0, 270.0, topFromScreen);
    float alpha = roundedBox(vUv, 0.018) * topVisibility;
