...
9. **P2 — too few examples shortened the experience.** The gallery now contains twenty-four entries: twelve image projects followed by twelve of the user's videos.
...
17. **Video content replacement:** Positions 13 through 24 now follow the exact supplied-file order and approved English copy. A content regression test locks the first twelve ids and image paths, verifies their approved content-based copy, and confirms none of those entries gained a video source.
...
24. **Sequence-number removal:** Numeric suffixes used only to enumerate repeated video-project types were removed from all visible titles: `Cinematic Study`, `Commercial Study`, `Game Cinematic`, and `Comic Drama Study` now display without `01`, `02`, or `03`.
25. **Image-project copy:** The original agency/client labels beneath the first twelve images were replaced with the approved content-based English copy. Titles are limited to one or two words, descriptions remain short, ids/image paths/order are unchanged, and no media or interaction behavior changed.
...
- **Copy and content:** The main gallery title is the approved `Selected Works`. The first twelve image projects use content-based English names from `Blue Portal — Digital Dreamscape` through `Crystal Machine — Technology Motion`. The final twelve retain their approved English descriptions and omit sequence-number suffixes from repeated title groups. The user-requested studio brand, top navigation, menu, and category/count row remain removed. The contact finale keeps `高振翔`, `AIGC CREATOR`, `电话 13293941800`, and `邮箱 13293941800@163.com`.
...
- Latest image-copy capture: 1/1 passed at the matched Chrome recording viewport; the first four new labels are readable, aligned, and free of wrapping or overlap.
